...
def get_ticker_options(market_type="India"):
    ...
    if market_type == "India":
        ...
        suffix_info = " (.NS for NSE, .BO for BSE)"
    else:
        categories = INTERNATIONAL_TICKERS
        suffix_info = " (US stocks)"
    ...
    return sorted(options, key=lambda x: x["label"]), suffix_info
...
with st.sidebar:
    ...
    # Get ticker options based on market type
    all_options, suffix_info = get_ticker_options(st.session_state.market_type)
    ...
    #     else:
    #         st.caption("Enter any valid stock ticker (e.g., AAPL, MSFT)")
